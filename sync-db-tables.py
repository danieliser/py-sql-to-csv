...
def get_last_id(table_info):
    if os.path.exists(get_output_filename(table_info)):
        return pd.read_csv(get_output_filename(table_info))['id'].max()
    elif 'last_id' in table_info:
        ...
        with open(csv_filename, 'a', newline='') as csv_file:
            ...
            with tqdm(total=rows_to_extract) as pbar:
                ...
                while offset < rows_to_extract:
                    if starting_id:
                        print('SELECT * FROM {} WHERE id > {} LIMIT {} OFFSET {}'.format(table_name, starting_id, limit, offset))
                        rows = run_query('SELECT * FROM {} WHERE id > {} LIMIT {} OFFSET {}'.format(table_name, starting_id, limit, offset))
                    else:
                        print( 'SELECT * FROM {} LIMIT {} OFFSET {}'.format(table_name, limit, offset))
                    ...
                    print()
                    # print Last synced id: 12345, transfered 1000 rows out of 10000
                    print( 'Last synced id: {}, transfered {} rows out of {}'.format(last_id, len(rows), rows_to_extract) )
                    print()
